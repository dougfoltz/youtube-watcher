#!/usr/bin/env python3
"""Organize a downloaded sermon into <Series>/<Sermon>/ under the Past Sermons folder.

Moves a freshly downloaded service video into a tidy folder tree inside
the church's Past Sermons folder in Files (config: output_dir) and writes a
machine-readable sermon.meta.json alongside it, so other agents can find
the video, its series, and its metadata without re-parsing YouTube.
...
from church_video_watcher import load_json, resolve_dir, CONFIG_PATH  # noqa: E402

# Empty: output_dir is already the Past Sermons folder, so series folders sit
# directly inside it. Set sermons_root in config.json to nest them under
# an extra subfolder.
DEFAULT_SERMONS_ROOT = ""
...
def main():
    ...
    # resolve_dir (not bare expanduser) so output_dir resolves identically
    # here and in the watcher, including under a scheduled run.
